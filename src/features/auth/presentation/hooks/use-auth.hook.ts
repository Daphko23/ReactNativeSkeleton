...
function mapAuthErrorToUserMessage(error: Error | string, t: any): string {
  ...
  if (
    ...
    return t('auth.loginScreen.errors.userNotFound', {
      defaultValue:
        'Benutzer wurde nicht gefunden. Bitte überprüfen Sie Ihre Angaben.',
    });
  }
  ...
  if (
    ...
    return t('auth.registerScreen.errors.weakPassword', {
      defaultValue:
        'Ihr Passwort ist zu schwach. Bitte wählen Sie ein stärkeres.',
    });
  }
...
export const useAuth = (): UseAuthReturn => {
  ...
  const [currentFlowState, setCurrentFlowState] = _useState<AuthFlowState>(
    AuthFlowState.IDLE
  );
    ...
    queryFn: async (): Promise<AuthUser | null> => {
      ...
      try {
        ...
        if (authContainer.isReady()) {
          ...
          logger.info(
            'Current user fetched successfully (Champion)',
        ...
        logger.error(
          'Failed to fetch current user (Champion)',
          LogCategory.BUSINESS,
          {},
          error as Error
        );
  ...
  const capabilitiesQuery = useQuery({
    ...
    queryFn: async (): Promise<FlowCapabilities> => {
      logger.info(
        'Fetching auth flow capabilities (Integrated)',
        LogCategory.BUSINESS
      );
      ...
      try {
        ...
        logger.info(
          'Auth flow capabilities fetched successfully (Integrated)',
          LogCategory.BUSINESS,
          {
            metadata: {
        ...
        logger.error(
          'Failed to fetch auth flow capabilities (Integrated)',
          LogCategory.BUSINESS,
          {},
          error as Error
        );
  ...
  const loginMutation = useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        // 🔒 GDPR Audit: Success
        await authGDPRAuditService.logLoginSuccess(user, 'email', {
          correlationId,
        });

        logger.info(
          'Login completed successfully (Champion)',
          LogCategory.BUSINESS,
          {
            correlationId,
            metadata: {
        ...
        if (isBusinessError(error as Error)) {
          logger.warn(
            'Login failed - Business Error (Champion)',
            LogCategory.BUSINESS,
            {
              correlationId,
              metadata: {
          ...
          logger.error(
            'Login failed - Technical Error (Champion)',
            LogCategory.BUSINESS,
            {
              correlationId,
              metadata: {
    ...
    onMutate: async () => {
      ...
      logger.info(
        'Login optimistic update started (Champion)',
        LogCategory.BUSINESS
      );
    ...
    onSuccess: user => {
      // 🔥 NAVIGATION FIX: Sofortige Auth State Synchronisation
      queryClient.setQueryData(authQueryKeys.user(), user);

      // 🔥 FORCE IMMEDIATE QUERY REFETCH für sofortige Navigation
      ...
      logger.info(
        'Login optimistic update completed (Champion)',
        LogCategory.BUSINESS,
        {
          metadata: {
            userId: user.id,
    ...
    onError: (error, variables, context) => {
      ...
      if (isBusinessError(error as Error)) {
        logger.warn(
          'Login optimistic update failed, reverted - Business Error (Champion)',
          LogCategory.BUSINESS,
          {
            metadata: {
              errorType: (error as Error).constructor.name,
        ...
        logger.error(
          'Login optimistic update failed, reverted - Technical Error (Champion)',
          LogCategory.BUSINESS,
          {
            metadata: {
              errorType: (error as Error).constructor.name,
  ...
  const registerMutation = useMutation({
    mutationFn: async ({
      ...
      logger.info(
        'Starting registration process (Champion)',
        LogCategory.BUSINESS,
        {
          metadata: {
            email,
      ...
      logger.info(
        'Registration completed successfully (Champion)',
        LogCategory.BUSINESS,
        {
          metadata: {
            userId: user.id,
    ...
    onSuccess: user => {
      ...
      logger.info(
        'Registration optimistic update completed (Champion)',
        LogCategory.BUSINESS,
        {
          metadata: {
            userId: user.id,
  ...
  const logoutMutation = useMutation({
    mutationFn: async (): Promise<void> => {
      ...
      // 🔒 GDPR Audit: Logout
      if (currentUser) {
        await authGDPRAuditService.logLogout(currentUser.id, 'user_initiated', {
          correlationId,
        });
      }

      logger.info(
        'Logout completed successfully (Champion)',
        LogCategory.BUSINESS,
        {
          correlationId,
          metadata: {
    ...
    onMutate: async () => {
      ...
      logger.info(
        'Logout optimistic update started (Champion)',
        LogCategory.BUSINESS
      );
    ...
    onSuccess: () => {
      ...
      logger.info(
        'Logout optimistic update completed (Champion)',
        LogCategory.BUSINESS
      );
    },
    ...
    onError: (error, variables, context) => {
      ...
      logger.error(
        'Logout optimistic update failed, reverted (Champion)',
        LogCategory.BUSINESS,
        {},
        error as Error
      );
  ...
  const registerError = registerMutation.error
    ? mapAuthErrorToUserMessage(registerMutation.error, t)
    : null;
  ...
  const startLogin = useCallback(
    async (credentials: LoginCredentials): Promise<void> => {
      ...
      try {
        ...
        logger.info(
          'Flow login completed successfully (Integrated)',
          LogCategory.BUSINESS,
          {
            metadata: {
              userId: result.id,
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Login flow failed';
  ...
  const startRegister = useCallback(
    async (data: RegisterData): Promise<void> => {
      ...
      try {
        ...
        logger.info(
          'Flow register completed successfully (Integrated)',
          LogCategory.BUSINESS,
          {
            metadata: {
              email: data.email,
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Register flow failed';
        setFlowError(errorMessage);
        setCurrentFlowState(AuthFlowState.ERROR);
        throw error;
      } finally {
