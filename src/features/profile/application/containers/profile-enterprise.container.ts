/**
 * @fileoverview Profile Enterprise DI Container - Application Layer
 * 
...
import { UploadAvatarUseCase } from '../use-cases/avatar/upload-avatar.use-case';
import { DeleteAvatarUseCase } from '../use-cases/avatar/delete-avatar.use-case';
...
/**
 * 🎯 PROFILE ENTERPRISE DI CONTAINER
 * 
 ...
 */
export class ProfileEnterpriseContainer implements IProfileServiceRegistry {
  private static instance: ProfileEnterpriseContainer | null = null;
   ...
   */
  static getInstance(config?: ProfileContainerConfig): ProfileEnterpriseContainer {
    if (!ProfileEnterpriseContainer.instance) {
      if (!config) {
        throw new Error('ProfileEnterpriseContainer requires configuration for first initialization');
      }
      ProfileEnterpriseContainer.instance = new ProfileEnterpriseContainer(config);
    }
    return ProfileEnterpriseContainer.instance;
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      this.log('Initializing Profile Enterprise Container...');
      ...
      if (healthStatus.overall === 'unhealthy') {
        throw new Error('Container initialization failed: unhealthy services detected');
      }

      this.isInitialized = true;
      this.log('Profile Enterprise Container initialized successfully');
    } catch (error) {
  ...
  async healthCheck(): Promise<ServiceHealthStatus> {
    ...
    try {
      ...
      const repoHealth = await this.accountSettingsRepository.healthCheck();
      
      // Determine overall health
      const services = {
        accountSettingsRepository: repoHealth.status as any,
        database: repoHealth.connectionStatus ? 'healthy' as const : 'unhealthy' as const,
      ...
      return {
        overall: 'unhealthy',
        services: {
          accountSettingsRepository: 'unhealthy',
          database: 'unhealthy',
          cache: 'unhealthy'
        },
        timestamp: new Date(),
  ...
  async dispose(): Promise<void> {
    ...
    try {
      this.log('Disposing Profile Enterprise Container...');
      ...
      ProfileEnterpriseContainer.instance = null;

      this.log('Profile Enterprise Container disposed successfully');
    } catch (error) {
  ...
  private validateConfiguration(): void {
    if (!this.config.supabaseClient) {
      throw new Error('ProfileEnterpriseContainer requires a Supabase client');
    }
  ...
  private log(message: string): void {
    if (this.config.enableLogging) {
      console.log(`[ProfileEnterpriseContainer] ${message}`);
    }
  }

  private logError(message: string, error?: any): void {
    if (this.config.enableLogging) {
      console.error(`[ProfileEnterpriseContainer] ${message}`, error);
    }
  }
}
 ...
 */
let globalContainer: ProfileEnterpriseContainer | null = null;

/**
 * Initialize global container
 */
export const initializeProfileContainer = async (config: ProfileContainerConfig): Promise<void> => {
  globalContainer = ProfileEnterpriseContainer.getInstance(config);
 ...
 */
export const getProfileContainer = (): ProfileEnterpriseContainer => {
  if (!globalContainer) {
