...
// import { useProfileContainer as _useProfileContainer } from '../../application/di/profile.container';
 ...
 * - Enterprise Logging: Essential audit trails
 */
export const useSocialLinksEdit = (
  params?: { navigation?: any } | string
): UseSocialLinksEditReturn => {
  ...
  // Temporarily disabled - DI Container integration needs to be fixed
  // const { container, accessor: _accessor } = _useProfileContainer();
  const updateSocialLinksUseCase = useMemo((): any => {
    try {
      // return container?.get('UpdateSocialLinksUseCase') || null;
      return null; // Temporarily disabled
    } catch {
      logger.error('Failed to get UpdateSocialLinksUseCase');
      return null;
    }
  }, []);

  const validateSocialLinksUseCase = useMemo((): any => {
    try {
      // return container?.get('ValidateSocialLinksUseCase') || null;
      return null; // Temporarily disabled
    } catch {
      logger.error('Failed to get ValidateSocialLinksUseCase');
      return null;
    }
  }, []);
  ...
  const updateSocialLink = useCallback(
    (platform: SocialPlatformKey, url: string) => {
      logger.info('Updating social link', LogCategory.BUSINESS, {
        userId,
        metadata: { platform, hasUrl: !!url },
      });
  ...
  const setVisibilityHandler = useCallback(
    (newVisibility: 'public' | 'friends' | 'private') => {
      logger.info('Updating social links visibility', LogCategory.BUSINESS, {
        userId,
  ...
  const validateSocialLinks = useCallback(
    async (links: SocialLink[]): Promise<Record<string, string>> => {
      try {
        const result = validateSocialLinksUseCase
          ? await validateSocialLinksUseCase.execute(
              {
        ...
        if (result && result.isValid) {
          logger.info(
            'Social links validation completed',
          ...
          (result.errors || []).forEach((error: any, _index: number) => {
        ...
        logger.error(
          'Failed to validate social links',
          LogCategory.BUSINESS,
          { metadata: { userId, linksCount: links.length } },
          _error as Error
        );
        return {};
      }
    },
    [validateSocialLinksUseCase, userId]
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: SocialLinksFormData) => {
      ...
      const result = updateSocialLinksUseCase
        ? await updateSocialLinksUseCase.execute({
            userId: userId || '',
  ...
  const reset = useCallback(() => {
    logger.info('Resetting social links form', LogCategory.BUSINESS, {
      userId,
    });
  ...
  const openSocialLink = useCallback(
    (platform: SocialPlatformKey) => {
      ...
      if (link?.url) {
        logger.info('Opening social link', LogCategory.BUSINESS, {
          userId,
  ...
  const getSocialLinkValue = useCallback(
    (platform: SocialPlatformKey) => {
      const link = socialLinks.find(l => l.platform === platform);
      return link?.url || '';
    },
