...
export const ProfileNavigation: React.FC<ProfileNavigationProps> = React.memo(({
  ...
  const accountActions: ActionItem[] = React.useMemo(() => {
    ...
    // Account Settings - only if enabled
    ...
    // Privacy Settings - only if enabled
    ...
    return actions;
  }, [isScreenEnabled, t, testIds, isLoading]);
  ...
  const profileActions: ActionItem[] = React.useMemo(() => {
    ...
    // Skills Management - only if enabled
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Account Section - only show if has actions */}
      {accountActions.length > 0 && (
        <>
          ...
          {/* Only show divider if both sections have actions */}
          {profileActions.length > 0 && <Divider style={styles.divider} />}
        </>
      )}

      {/* Profile Section - only show if has actions */}
